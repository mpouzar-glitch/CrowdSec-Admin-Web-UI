...
function formatRelativeTime(dateString) {
    ...
    if (diffMins < 1) return 'právì teï';
    if (diffMins < 60) return `pøed ${diffMins} min`;
    if (diffHours < 24) return `pøed ${diffHours} h`;
    return `pøed ${diffDays} dny`;
}
...
async function apiGet(endpoint) {
    try {
        ...
        showNotification('Chyba pøi naèítání dat: ' + error.message, 'error');
        throw error;
    }
}
...
async function apiPost(endpoint, data) {
    try {
        ...
        console.error('API Error:', error);
        showNotification('Chyba pøi odesílání dat: ' + error.message, 'error');
        throw error;
    }
}
...
async function apiDelete(endpoint) {
    try {
        ...
        console.error('API Error:', error);
        showNotification('Chyba pøi mazání: ' + error.message, 'error');
        throw error;
    }
}
...
function showAlertModal(alert) {
    ...
    detail.innerHTML = `
        ...
        <div class="alert-detail-grid">
            ...
            <div class="detail-item">
                <label>Èas vytvoøení</label>
                <div class="value">${formatDate(alert.created_at)}</div>
            </div>
            <div class="detail-item">
                <label>Poèet událostí</label>
...
async function deleteAlert(id) {
    ...
    try {
        ...
        showNotification('Alert byl úspìšnì smazán', 'success');
        loadAlerts();
    } catch (error) {
...
async function deleteDecision(id) {
    ...
    try {
        ...
        showNotification('Ban byl úspìšnì smazán', 'success');
        loadDecisions();
    } catch (error) {
    ...
    if (addForm) {
        addForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                showNotification('Ban byl úspìšnì pøidán', 'success');
